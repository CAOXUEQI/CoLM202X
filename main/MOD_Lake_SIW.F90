...
! ---------------- local variables in lake/snow/soil temperature calculation --------------
  ...
  real(r8) phid(nl_lake)
  real(r8) phiz(nl_lake)
...
!  real(r8) savedtke1
!----------------------------------SIW--------------------------------------
  real(r8) g_wvl(8) = (/0.25d0, 0.70d0, 1.41d0, 1.90d0, 2.50d0, 3.50d0, 4.00d0, 5.00d0/)
  real(r8) g_wvl_center(7)
  real(r8) g_b0(7), g_b1(7), g_b2(7)
  real(r8) g_F07_c2(7), g_F07_c1(7), g_F07_c0(7)
  real(r8) g_F07_p2(7), g_F07_p1(7), g_F07_p0(7)
      ...
      IF (.not. DEF_USE_SNICAR .or. present(urban_call)) THEN
         ...
         ELSE IF(DEF_USE_TWOSTREAM)THEN
            ...
            if (snl == 0) then
               ...
               ! print*,'f*w0/(4*pi)',f*w0/(4*pi),'1-3*g*(1.0/(3**0.5))*coszen',1-3*g*(1.0/(3**0.5))*coszen
               ! print*,'s1 s2',s1,s2 !right
               k=((1.0-w0)*(1.0-w0*g)/(1.0/3.0))**0.5
               ! print*,'k',k !right
               ! print*,(1.0-w0)/(1.0-w0*g)
               ssa=((1.0-w0)/(1.0-w0*g))**0.5
               ! print*,'ssa',ssa,(1.0-w0)/(1.0-w0*g),1.0-w0*g !right
               z1=-((1.0-w0*g)*(s2+s1))/(1.0/3.0)+(s2-s1)/(1.0/(3**0.5)*coszen) 
               z2=-((1.0-w0)*(s2-s1))/(1.0/3.0)+(s2+s1)/(1.0/(3**0.5)*coszen) 
               ! print*,'z1 z2',z1,z2 !right 
               saa=z1*(coszen**2.0)/(1.0-(coszen**2.0)*(k**2.0)) !right
               ! print*,z1*(coszen**2),(1-(coszen**2)*(k**2)),z2*(coszen**2)
               sbe=z2*(coszen**2.0)/(1.0-(coszen**2.0)*(k**2.0)) !right
               ! print*,'saa sbe',saa,sbe !right
               se=(saa+sbe)/2.0
               sr=(saa-sbe)/2.0
               sv=(1.+ssa)/2.0
               su=(1.-ssa)/2.0
               ...
               ! print*,'f',f,coszen**2*w0*f/(1-(coszen**2)*(k**2))
               ! print*,'k2',k2,'sv2',sv2,'su2',su2,&
               !        'se2',se2,'sr2',sr2,'sk2',sk2,'sh2',sh2
               !print*,se*su,-tao(nl_lake+1)/coszen,exp(-tao(nl_lake+1)/coszen),se*su*exp(-tao(nl_lake+1)/coszen),sv**2*exp(k*tao(nl_lake+1)),su**2*exp(-k*tao(nl_lake+1))
               !print*,'ssk',ssk,'ssh',ssh
               ! print*,'eup',ssk*su*exp(k*tao(nl_lake+1))+ssh*sv*exp(-k*tao(nl_lake+1))+sr*exp(-tao(nl_lake+1)/coszen)
               ! print*,'edown',ssk*sv*exp(k*tao(nl_lake+1))+ssh*su*exp(-k*tao(nl_lake+1))+se*exp(-tao(nl_lake+1)/coszen)
               ...
               do j = 1, nl_lake+1
                  ...
                  !        -2.0*pi*(1.0/(3**0.5))*(ssk*sv*exp(k*tao(j))+ssh*su*exp(-k*tao(j))+se*ttu)
                  ! print*,'ssk*su*exp(k*tao(j))',ssk*su*exp(k*tao(j))
                  ! print*,'ssk*sv*exp(k*tao(j))',ssk*sv*exp(k*tao(j))
                  ! !fangan2
                  ...
                  eupd(j)=su2*sk2*exp(k2*tao(j))+sv2*sh2*exp(-k2*tao(j))+sr2*ttu&
                         -(sv2*sk2*exp(k2*tao(j))+su2*sh2*exp(-k2*tao(j))+se2*ttu)
                  eupz(j)=sabgv*ttu                  
                  ! print*,'eup',su2*sk2*exp(k2*tao(j))+sv2*sh2*exp(-k2*tao(j))+sr2*ttu
                  ! print*,'edown',sv2*sk2*exp(k2*tao(j))+su2*sh2*exp(-k2*tao(j))+se2*ttu
                  ! print*,'edowns',sabgv*ttu
                  ! print*,'eall',j,eup(j)
                  ! print*,'eup',j,2.0*pi*(1.0/(3**0.5))*(ssk*sv*exp(k*tao(j))+ssh*su*exp(-k*tao(j))+se*ttu)
                  ! print*,'edown',j,2.0*pi*(1.0/(3**0.5))*(ssk*su*exp(k*tao(j))+ssh*sv*exp(-k*tao(j))+sr*ttu)&
                  !                  +sabgv*ttu
                  ! print*,'edowns',coszen*sabgv*ttu
                  ! print*,'eup',j,sv2*sk2*exp(k2*tao(j))+su2*sh2*exp(-k2*tao(j))+se2*ttu
                  ! print*,'edown',j,su2*sk2*exp(k2*tao(j))+sv2*sh2*exp(-k2*tao(j))+sr2*ttu+coszen*f*ttu
               end do 
               sum = 0
               do j = 1,nl_lake+1
               !    ttu=exp(-tao(j)/coszen)
               ...
               ! do j = 1, nl_lake
               !    phid(j)=eupd(j)-eupd(j+1)
               !    print*,'phid',j,phid(j)
               ! end do 
               ! do j = 1, nl_lake
               !    phiz(j)=eupz(j)-eupz(j+1)
               !    print*,'phiz',j,phiz(j)
               ! end do
               ! do j = 1, nl_lake
               !    ! print*,'edownz',j,eupz(j)
               ! end do
               ! sum2=0
               ! do j = 1, nl_lake+1
               !    sum2=sum2+eup(j)
               ...
               !    if (j == nl_lake) phi_soil = eup(j+1)
               ! end do
               ! print*,'sum2',sum2,sabgv+sabgvd
               ! print*,sabgvdu,2.0*pi*(1.0/(3**0.5))*(ssk*sv+ssh*su+se)
               ! print*,sum+2.0*pi*(1.0/(3**0.5))*(ssk*sv+ssh*su+se)+&
               !          2.0*pi*(1.0/(3**0.5))*(ssk*su*exp(k*tao(nl_lake+1))+ssh*sv*exp(-k*tao(nl_lake+1))&
               !          +sr*exp(-tao(nl_lake+1)/coszen))&
               !          +coszen*f*exp(-tao(nl_lake+1)/coszen),&
               !             sabg*(1-betaprime)
               ! print*,'check energy',sum+sv2*sk2+su2*sh2+sk2*su2*exp(k*tao(nl_lake+1))+sh2*sv2*exp(-k*tao(nl_lake+1))&
               !          +sr2*exp(-tao(nl_lake+1)/coszen)&
               !          +sabgv*exp(-tao(nl_lake+1)/coszen),&
               !             sabg*(1-betaprime)
               print*,'bj',sabgvd,eupd(1),sv2*sk2*exp(k2*tao(11))+su2*sh2*exp(-k2*tao(11))+se2*tu
               ! print*,eupd(11),su2*sk2*exp(k2*tao(nl_lake+1))+sv2*sh2*exp(-k2*tao(nl_lake+1))+sr2*exp(-tao(nl_lake+1)/coszen)
               ! print*,'bj',sabgvd,eupd(1),2.0*pi*(1.0/(3**0.5))*(sv*ssk*exp(k*tao(11))+su*ssh*exp(-k*tao(11))+se*tu)
               ...
               ! print*,'check energy','snl',snl,'beta',betaprime,'all',sum+&  !2.0*pi*(1.0/(3**0.5))*(sv*ssk+su*ssh+se)+
               !           2.0*pi*(1.0/(3**0.5))*(ssk*su*exp(k*tao(nl_lake+1))+ssh*sv*exp(-k*tao(nl_lake+1))&
               !          +sr*exp(-tao(nl_lake+1)/coszen))&
               !          +sabgv*exp(-tao(nl_lake+1)/coszen),&
               !             'in',sabgv+sabgvd,'sabg',sabg,sabgn+sabgnd+sabgv+sabgvd                
               ! print*,sum
               ! print*,f
               ! print*,2.0*pi*(1.0/(3**0.5))*(ssk*sv*exp(k*tao(1))+ssh*su*exp(-k*tao(1))+se*exp(-tao(1)/coszen)),&
               !        sabgvdu
               ! print*,2.0*pi*(1.0/(3**0.5))*(ssk*su*exp(k*tao(1))+ssh*sv*exp(-k*tao(1))+sr*exp(-tao(1)/coszen)),&
               !        sabgvda
               ! print*,2.0*pi*(1.0/(3**0.5))*(ssk*sv*exp(k*tao(1))+ssh*su*exp(-k*tao(1))+se*exp(-tao(1)/coszen))&
               !       +2.0*pi*(1.0/(3**0.5))*(ssk*su*exp(k*tao(nl_lake+1))+ssh*sv*exp(-k*tao(nl_lake+1))+sr*exp(-tao(nl_lake+1)/coszen))&
               !       +coszen*f*exp(-tao(1)/coszen)
            else   ! snow covered, this should be improved upon; Mironov 2002 suggests that SW can penetrate thin ice and may
                  ! cause spring convection.
               phi(:) = 0.
               phi_soil = 0.

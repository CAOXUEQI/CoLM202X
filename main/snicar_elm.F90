   ...
   subroutine SNICAR_AD_RT (flg_snw_ice, bounds, num_nourbanc, filter_nourbanc,  &
     ...
     real(r8)          , intent(in)  :: h2osoi_liq     ( bounds%begc: , -nlevsno+1: )      ! +CAW
     ...
     real(r8):: &
        ...
        trntdr(-nlevsno+1:nlevice+1)  , & ! total transmission to direct beam for layers above
        trndif(-nlevsno+1:nlevice+1)  , & ! diffuse transmission to diffuse beam for layers above
        rupdir(-nlevsno+1:nlevice+1)  , & ! reflectivity to direct radiation for layers below
        rupdif(-nlevsno+1:nlevice+1)  , & ! reflectivity to diffuse radiation for layers below
     ...
     associate(&
       ...
       ! Get current timestep---------------different---------------------
       nstep = get_nstep()
      ...
      ! (when called from CSIM, there is only one column)
       do fc = 1,num_nourbanc
         ...
         ! set snow/ice mass to be used for RT:
          ...
          ! CAW - zero mass aerosol concentration over snow & ice layers 
          do i=-nlevsno+1,snl_btm,1
                mss_cnc_aer_lcl(i,:) = 0._r8 
          enddo
          
          !+CAW - seperate out the flx variable that is used elsewhere in the model  
          do i=-nlevsno+1,1,1
             flx_abs(c_idx,i,:) = 0._r8
          enddo 
          
          ! FIRST DETERMINE ALBEDO OF BARE ICE 
   ...
   !H. Wang
               ...
               !       write(iulog,*) "CAW c",c_idx,"bnd",bnd_idx,"layer",i,"flx_abs_lcl",flx_abs_lcl(i,bnd_idx)
...
!                endif
             ...
             bare_ice_albout(c_idx,1) = albout_lcl(1)
             flx_sum         = 0._r8
       ...
       endif ! if there is bare land ice 
       !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
             ...
             if (flg_snw_ice == 1) then
               ...
               ! This layer is presumed to have the fresh snow effective radius.
                if (snl(c_idx) > -1) then
                  ...
                  flg_nosnl                    =  0
                  snl_lcl                      =  snl(c_idx) 
                  h2osno_liq_lcl(-nlevsno+1:0) =  h2osno_liq(c_idx,:)
                  h2osno_ice_lcl(-nlevsno+1:0) =  h2osno_ice(c_idx,:)
                  snw_rds_lcl(-nlevsno+1:0)    =  snw_rds(c_idx,:)
                endif
                  ...
                  if (atm_type_index == atm_type_default) then
                     ...
                     if ( (use_snicar_lndice).and. (lnd_ice ==1) ) then !+CAW
                        ...
                        if (1==1) then ! new weighting scheme to fix NIR 
                            ...
                            flx_wgt(5)  = 0.199547150388836_r8 - 0.05_r8
                        endif
                     endif                                              !+CAW
                  else                 
   ...
   !H. Wang
             ...
             if ( (use_snicar_lndice) .and. (lnd_ice ==1) ) then
                ...
                flx_abs(c_idx,:,1) = flx_abs_lcl(-nlevsno+1:1,1)
             endif
             ...
             !write(iulog,*) "CAW c",c_idx,"flx_sum2",flx_sum2
             !write(iulog,*) "CAW c",c_idx,"sum(flx_sum2(1:snl_btm))",sum(flx_sum2(1:snl_btm)) 
             if ( (use_snicar_lndice) .and. (lnd_ice ==1) ) then
                flx_abs(c_idx, -nlevsno+1:0, 2) = flx_sum2(-nlevsno+1:0)
                flx_abs(c_idx, 1, 2) = sum(flx_sum2(1:snl_btm+1))
             else 
          ...
          !      write(iulog,*)"CAW c",c_idx,"flx_abs(c_idx,:,2)",flx_abs(c_idx,:,2)
           ...
           !     write(iulog,*)"CAW c",c_idx,"flx_abs_lcl(:,4)",flx_abs_lcl(:,4)
           !     write(iulog,*)"CAW c",c_idx,"flx_abs_lcl(:,5)",flx_abs_lcl(:,5)
            !    do i=snl_top,snl_btm,1
            !         do bnd_idx= 1,numrad_snw
            !           write(iulog,*)"CAW c",c_idx,"bnd",bnd_idx,"i",i,"flx_abs_lcl",flx_abs_lcl(i,bnd_idx)    
            !         enddo
            !          
